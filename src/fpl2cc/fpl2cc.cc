  ...
  For abstracting implementations:
    - reducers:
      ...
      - integrate reducers (generate_reduce_functions)
        o abstracted impls
        - folding ProdExprs:
            what does folding mean?  it means replacing a given ProdExpr
            with another one
            a given rule produces with the thing it's produced from.
            (or maybe it's the step which gets folded, not the rule -
            consider: 'x' -> bar ; moo bar -> bat +{ ... }+
            the argument bar would fold, making moo bar -> bat into
            moo 'x' -> bat +{ ... }+
            So to be foldable, a ProdExpr must:
             - not be generated by any rules with more than one
               non-ejected step
             - not be generated by any rules with explicit or abstracted
               reduction code
             - not be foldable into more than one type (i.e. all possible
               sources must eventually fold to either terminals or
               nonterminals
